...
from .exceptions import ExistingThumbnailFolderError;
from .exceptions import MissingThumbnailsError;

THUMB_DIM = 200; #The (square) thumbnail size
PAD = 20; #The padding between elements in the grid
X_CROP_START = 425; #The left edge of the scene to crop when creating thumbnails
Y_CROP_START = 600; #The top edge of the scene to crop when creating thumbnails
FONT_FACE = cv2.FONT_HERSHEY_COMPLEX_SMALL; #The font to use
TEXT_SCALE = 2; #The font size for the main text labels
LABEL_SCALE = 1; #The font size for the individual asset labels
FONT_THICKNESS = 1; #The font thickness to use
FONT_COLOUR = (0,0,0); #The colour for the labels (set to black)
TEXT_HEIGHT = 39; #The height of the main text labels
LABEL_HEIGHT = 20; #The height of the smaller asset labels
...
class Thumbnails: 
    ...
    def __init__(self,image_dir,controller=None):
        ...
        """
        self.items = Items(); #Get the metadata for the items
        ...
        if not os.path.exists(self.thumb_dir):
            ...
            if controller is None:
                raise MissingThumbnailsError("Thumbnail folder not found. Provide a controller for a blank iTHOR scene to generate thumbnails.");
            else:
    ...
    def generate_grid(self,objects,mats,display_names=False):
        ...
        cv2.putText(img=grid, text='Objects:', org=self.obj_text_pos, fontFace=FONT_FACE, fontScale=TEXT_SCALE, color=FONT_COLOUR, thickness=FONT_THICKNESS);
        cv2.putText(img=grid, text='Mats:', org=self.mat_text_pos, fontFace=FONT_FACE, fontScale=TEXT_SCALE, color=FONT_COLOUR, thickness=FONT_THICKNESS);
        ...
        for i,obj in enumerate(objects):
            ...
            cv2.putText(img=grid, text=label, org=(left,bottom+LABEL_HEIGHT), fontFace=FONT_FACE, fontScale=LABEL_SCALE, color=FONT_COLOUR, thickness=FONT_THICKNESS);
    ...
    def get_grid_size(self,objects,mats):
        ...
        self.height = PAD + TEXT_HEIGHT + (THUMB_DIM + LABEL_HEIGHT) * obj_height + PAD * (obj_height + 1);
        self.height += TEXT_HEIGHT + (THUMB_DIM + LABEL_HEIGHT) * mat_height + PAD * (mat_height + 1);
        
        #The position of the two main text labels
        self.obj_text_pos = (PAD, PAD+TEXT_HEIGHT);
        self.mat_text_pos = (PAD, PAD+TEXT_HEIGHT*2+PAD*(obj_height + 1)+(THUMB_DIM+LABEL_HEIGHT)*obj_height);
    ...
    def generate_thumbnails(self,blank_scene_controller):
        ...
        try:
            ...
            raise ExistingThumbnailFolderError("A thumbnail folder already exists, please delete it and try again.");
        ...
        for asset in self.items.assets:
            ...
            cropped_img = img[X_CROP_START:(X_CROP_START + THUMB_DIM),Y_CROP_START:(Y_CROP_START + THUMB_DIM)];
            #Write the image to file
            cv2.imwrite(os.path.join(self.thumb_dir,'{}.png'.format(name)),cropped_img)
