...
class Table:
    ...
    def __init__(self,mats,goal_objects):
        """
        ...
        goal_objects : list of list of string
        ...
        if len(mats) != HORIZONTAL_SLOTS:
            raise GridCoordinateError("List of mats was {} wide, it should be {} wide.".format(len(mats),HORIZONTAL_SLOTS));
        if not all([len(s) == VERTICAL_SLOTS for s in mats]):
            raise GridCoordinateError("List of mats must be {} tall".format(VERTICAL_SLOTS));
        ...
        for x in range(HORIZONTAL_SLOTS):
            ...
            for y in range(VERTICAL_SLOTS):
                ...
                if goal_objects[x][y] is not None:
                    go = goal_objects[x][y];
                    if mats[x][y] is None:
                        raise NoMatError("Tried to place {} at position ({},{}) but there's no mat there.".format(go,x,y))
